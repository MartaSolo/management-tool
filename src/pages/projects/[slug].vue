...
watch(
  () => project.value?.name,
  () => {
    usePageStore().pageData.title = project.value?.name ? `Project ${project.value.name}` : ''
  },
)
...
<template>
  ...
  <section
    v-if="project && project.tasks.length > 0"
            ...
            <TableRow v-for="task in project.tasks" :key="task.id">
              <TableCell class="p-0">
                <RouterLink
                  ...
                  class="text-left hover:bg-muted p-4"
                >
                  {{ task.name }}
                </RouterLink>
              </TableCell>
...
.table-container {
  @apply overflow-hidden overflow-y-auto rounded-md bg-slate-900 min-h-80;
}
</style>
